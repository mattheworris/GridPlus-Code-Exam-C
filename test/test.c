...
#define tlv_print_hex print_hex
...
int main()
{
    ...
    int err = tlv_parse(t, &nTok, testDataShort, sizeof(testDataShort));
    printf(" - result: %i\n\r", err);
    assert(err == 12);
    ...
    err = tlv_serialize(tlvRaw, &tlvBufSz, t, nTok);
    printf(" - result: %i\n\r", err);
    assert(err == 12);
    ...
    err = tlv_parse(t, &nTok, testDataLong, sizeof(testDataLong));
    printf(" - result: %i\n\r", err);
    assert(err == 269);
